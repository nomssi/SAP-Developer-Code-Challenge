CLASS z2ui5_cl_app_demo_base64 DEFINITION PUBLIC.
  ...
  PROTECTED SECTION.
    ...
    METHODS ui5_render_popup_abap
      RETURNING
        VALUE(r_result) TYPE string.

  PRIVATE SECTION.
     methods STRING_TO_SOLI importing !IV_STRING type STRING
                                      iv_row_len TYPE i DEFAULT 255
                            returning value(rT_SOLI) type SOLI_TAB .

    METHODS serialize_data IMPORTING is_file_edit TYPE ty_file
                             RETURNING VALUE(result) TYPE string.
ENDCLASS.



CLASS Z2UI5_CL_APP_DEMO_BASE64 IMPLEMENTATION.
  ...
  METHOD string_to_soli.
    ...
    DATA(lv_size) = strlen( iv_string ).

    DATA(lv_rows) = lv_size DIV iv_row_len.
    DATA(lv_last_row_len) = lv_size MOD iv_row_len.

    DO lv_rows TIMES.
      ls_soli-line = iv_string+lv_off(iv_row_len).
  ...
  METHOD serialize_data.
    ...
    result = |METHOD image_{ is_file_edit-name }.\n|.
    result &&= |result = |.
    lt_soli = STRING_TO_SOLI( iv_string = is_file_edit-data
                              iv_row_len = 240 ).
    LOOP AT lt_soli INTO DATA(ls_soli).

      result &&=  lv_op && '`' && ls_soli-line && '`'.
      lv_op = | &&\n|.
    ENDLOOP.
    result &&= |.\nENDMETHOD.|.
  ENDMETHOD.
  ...
  METHOD ui5_render_popup_abap.
    ms_file_edit-abap = serialize_data( ms_file_edit ).
    ...
    DATA(lo_popup) = Z2UI5_CL_XML_VIEW=>factory_popup(
             ...
             title = 'Data:'
                 ...
                 width = '99%'
                 enabled = abap_false
                 value = client->_bind( ms_file_edit-abap )
         )->get_parent(
         )->footer( )->overflow_toolbar(
             )->toolbar_spacer(
             )->button(
  ...
  METHOD ui5_render_view_main.
    ...
    DATA(page) = Z2UI5_CL_XML_VIEW=>factory(  )->shell( )->page(
            title          = 'abap2UI5 - File Upload/Download Base64 converter'
            navbuttonpress = client->_event( 'BACK' )
            shownavbutton  = abap_true
        )->header_content(
            )->toolbar_spacer(
    ...
    DATA(tab) = page->table(
            ...
            items = client->_bind( mt_file )
                    ...
                    text = 'Base64 ABAP code'
                    press = client->_event( 'POPUP_ABAP' )
                )->button(
                    text = 'display'
  ...
  METHOD z2ui5_if_app~main.
    ...
    CASE app-view_popup.
      ...
      WHEN 'ABAP'.
        app-next-xml_popup = ui5_render_popup_abap( ).
    ENDCASE.
